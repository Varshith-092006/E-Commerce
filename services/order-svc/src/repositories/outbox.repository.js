...
export class OrderOutboxRepository {
  ...
  async claimBatch({ workerId, batchSize = 10, lockTimeoutMs = 30000, maxRetries = 5 }) {
    ...
    const candidates = await this.prisma.orderOutbox.findMany({
      where: {
        OR: [
          { status: 'PENDING' },
          {
    ...
    const updateResult = await this.prisma.orderOutbox.updateMany({
      where: {
        id: { in: candidateIds },
        OR: [
          { status: 'PENDING' },
    ...
    return await this.prisma.orderOutbox.findMany({
      where: {
        id: { in: candidateIds },
        locked_by: workerId,
        locked_at: now,
  ...
  async markProcessed(id) {
    return await this.prisma.orderOutbox.update({
      where: { id },
      data: {
        status: 'PROCESSED',
  ...
  async releaseExpiredLeases(lockTimeoutMs = 30000) {
    ...
    const result = await this.prisma.orderOutbox.updateMany({
      where: {
        status: 'PROCESSING',
        locked_at: { lt: staleThreshold },
      },
  ...
  async getPendingCount() {
    return await this.prisma.orderOutbox.count({
      where: {
