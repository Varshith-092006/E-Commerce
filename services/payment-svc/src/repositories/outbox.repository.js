...
export class PaymentOutboxRepository {
  ...
  async claimBatch({ workerId, batchSize = 10, lockTimeoutMs = 30000, maxRetries = 5 }) {
    ...
    const candidates = await this.prisma.paymentOutbox.findMany({
      where: {
        OR: [
          { status: 'PENDING' },
          {
    ...
    const updateResult = await this.prisma.paymentOutbox.updateMany({
      where: {
        id: { in: candidateIds },
        OR: [
          { status: 'PENDING' },
    ...
    return await this.prisma.paymentOutbox.findMany({
      where: {
        id: { in: candidateIds },
        locked_by: workerId,
        locked_at: now,
  ...
  async markProcessed(id) {
    return await this.prisma.paymentOutbox.update({
      where: { id },
      data: {
        status: 'PROCESSED',
  ...
  async releaseExpiredLeases(lockTimeoutMs = 30000) {
    ...
    const result = await this.prisma.paymentOutbox.updateMany({
      where: {
        status: 'PROCESSING',
        locked_at: { lt: staleThreshold },
      },
  ...
  /**
   * Creates an outbox event in payment_outbox within transaction or standard client
   */
  async createEvent(
    { eventType, aggregateType = 'Payment', aggregateId, payload, status = 'PENDING' },
    tx = this.prisma,
  ) {
    return await tx.paymentOutbox.create({
      data: {
  ...
  async getPendingCount() {
    return await this.prisma.paymentOutbox.count({
      where: {
...
export const paymentOutboxRepository = new PaymentOutboxRepository();
